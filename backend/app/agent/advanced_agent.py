...
## 分析步骤
1. **第一步**：[具体 SQL 思路，比如：用 dim_customer JOIN fct_orders GROUP BY customer_id 找出按 total_due 排序的前 N 名] — [为什么这样做]
2. **第二步**：[具体 SQL 思路，比如：用上一步的 id 去查 fct_order_details 获取产品明细] — [为什么这样做]
3. **第三步**：[具体 SQL 思路] — [为什么这样做]
...
