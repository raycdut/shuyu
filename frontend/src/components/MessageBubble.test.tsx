...
describe('MessageBubble', () => {
  it('renders user message right-aligned', () => {
    const { container } = render(<MessageBubble message={{ role: 'user', content: '你好' }} />)
  ...
  it('renders assistant message left-aligned', () => {
    const { container } = render(<MessageBubble message={{ role: 'assistant', content: '我是助手' }} />)
  ...
  it('shows agent name for assistant messages', () => {
    render(<MessageBubble message={{ role: 'assistant', content: '回答' }} />)
    expect(screen.getByText('Data Chat')).toBeInTheDocument()
  })

  it('renders markdown table', () => {
    const text = '| 产品 | 价格 |\n| --- | --- |\n| 苹果 | 5.0 |\n| 香蕉 | 3.5 |'
    render(<MessageBubble message={{ role: 'assistant', content: text }} />)
  ...
  it('renders regular text without markdown table', () => {
    render(<MessageBubble message={{ role: 'assistant', content: '这是一段普通文本。\n没有表格。' }} />)
